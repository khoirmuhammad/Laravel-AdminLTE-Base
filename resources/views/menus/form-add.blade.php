   ...
   <form id="menu-form">
        <div class="row">
          <div class="col-md-6">
              ...
              <div class="form-group">
                <label>Urutan</label>
                <input type="number" class="form-control" id="order" name="order" placeholder="Urutan Menu">
              </div>
          ...
          $('#cb-role-categories').append('<label>Pilih Role / Peran</label>');
            for(let i = 0; i < response.data.length; i++) {
                $("#cb-role-categories").append(`<div class="icheck-${i < color.length ? color[i] : color[i-color.length]}">
                    <input type="checkbox" id="role_cat_${response.data[i].id}" name="role_cat" value="${response.data[i].id}">
                    <label for="role_cat_${response.data[i].id}">
  ...
  function post_save_menu() {
    ...
    $.ajax({
       url:"/api/menu/save-menu",
        ...
        success: function(response){
            debugger;
            swal("Berhasil", `Menu ${response.data.title} berhasil ditambahkan`, "success");
